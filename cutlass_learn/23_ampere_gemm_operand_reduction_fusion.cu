...
// “PitchLinear” 直译为 “行距线性”，通常指一种以线性方式且按特定行距（pitch，这里可理解为相邻数据行之间的间隔）来组织数据的布局模式。
// 例如在二维数组存储时，每一行数据在内存中线性排列，并且行与行之间的间隔固定，以这种布局方式来方便数据的访问和处理。
...
// Pointers to A and its leading dimension should be divisible by this many elements (not bytes).
// This allows for vectorized memory accesses.
...
using EpilogueOp = cutlass::epilogue::thread::LinearCombination<
    // LinearCombination is a common epilogue that computes D = alpha * Accum + beta * C.
    ElementOutput, // Data type of output matrix.
    128 / cutlass::sizeof_bits<ElementOutput>::value, // The number of elements per vectorized.
    // memory access. This becomes the vector width of
    // math instructions in the epilogue too.
    ElementAccumulator, // Data type of accumulator
    ElementComputeEpilogue //  Data type for intermediate computations within the epilogue
>;
// This is the core CUTLASS component that defines the device-level
// GEMM operation capable of fusing the K-reduction.
...
/// When parallel_split_k is enabled and batch_count (representing split_k_slices) > 1,
/// the GEMM K-dimension is split into slices. Each slice is computed partially, and then
/// a separate reduction step is needed to combine these partial results.
/// This is handled by ReduceGemmSplitK for the main GEMM output
/// and ReduceVectorSplitK for the K-reduced vector output.


// Below is the reduction kernel used in the case of parallel split-k
using ReduceGemmSplitKShape = cutlass::MatrixShape<4, 64>; //  Defines the tile shape for the reduction kernel that combines partial GEMM results.

// Defines the reduction operation (e.g., sum) and the data types involved.
...
// Defines the kernel for reducing the intermediate GEMM results when split-K is used.
using ReduceGemmSplitKKernel = cutlass::reduction::kernel::ReduceSplitK<
    ReduceGemmSplitKShape,
    EpilogueOp,
    ReduceOp
>;
// The device-level reduction operator for GEMM output.
using ReduceGemmSplitK = cutlass::reduction::device::ReduceSplitK<ReduceGemmSplitKKernel>;

// Tile shape for reducing the K-vector partial sums.
...
using DummyEpilogueOp = cutlass::epilogue::thread::LinearCombination<
    ...
    cutlass::epilogue::thread::ScaleType::Nothing ///< Control Alpha and Beta scaling
>;
...
Result profile(Options const& options)
{
    ...
    ElementComputeEpilogue alpha = options.parallel_split_k
                                       ? ElementComputeEpilogue(1)
                                       : ElementComputeEpilogue(options.alpha);
    ElementComputeEpilogue beta = options.parallel_split_k
    ...
    // This structure holds all the runtime parameters needed to execute the GEMM kernel.
    Gemm::Arguments arguments(
        mode,
        options.problem_size, //  (M, N, K dimensions of the problem)
        batch_count, // Number of individual GEMM problems (used for batched GEMM or split-K).
        {alpha, beta}, //  A struct usually containing alpha and beta scaling factors.
        tensor_a.device_data(), // <- reference to tensor A on device
        ...
        tensor_reduction.layout().stride(0) // Leading dimension for the reduction vector.
    );
    ...
    if (options.parallel_split_k && batch_count > 1)
    {
        ...
        cutlass::TensorRef<ElementOutput, cutlass::layout::RowMajor> workspace_gemm_tensorref(
            static_cast<ElementOutput*>(workspace_gemm_ptr), splitk_gemm_layout);

        cutlass::TensorRef<ElementOutput, cutlass::layout::RowMajor> tensor_d_tensorref(
            tensor_d.device_ref().data(), splitk_gemm_layout);

        cutlass::TensorRef<ElementOutput, cutlass::layout::RowMajor> tensor_c_tensorref(
            tensor_c.device_ref().data(), splitk_gemm_layout);

        ReduceGemmSplitK::Arguments reduce_gemm_splitk_arguments{
            // For GEMM output reduction, this corresponds to N and M of the GEMM output.
            cutlass::MatrixCoord(options.problem_size.n(), options.problem_size.m()),
            batch_count,// Number of partial results to reduce
            size_t(options.problem_size.m() * options.problem_size.n()),
            workspace_gemm_tensorref, // to the workspace where partial sums from the GEMM stage were stored.
            tensor_d_tensorref, // the final output tensor
            tensor_c_tensorref,
            {alpha, beta}
        };
        ...
        ElementOutput* workspace_vector_ptr = static_cast<ElementOutput*>(workspace_gemm_ptr) + batch_count * options.
            problem_size.m() * options.problem_size.n();
        cutlass::TensorRef<ElementOutput, cutlass::layout::RowMajor> workspace_vector_tensorref(
            workspace_vector_ptr, splitk_vector_layout);

        cutlass::TensorRef<ElementOutput, cutlass::layout::RowMajor> tensor_reduction_tensorref(
