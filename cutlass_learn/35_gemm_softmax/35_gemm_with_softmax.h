...
#include "35_gemm_with_epilogue_visitor.h"
...
namespace cutlass
{
    namespace kernel
    {
        ...
        public:
            ...
            using TensorRefD = TensorRef<ElementD_, Layout>;
            using TensorRefN = TensorRef<ElementNorm_, Layout>;
            using TensorRefSum = TensorRef<ElementSum_, Layout>;
            using TensorRefSoft = TensorRef<ElementSoft_, Layout>;
            ...
            CUTLASS_DEVICE void operator()(Params const& param, SharedStorage& shared_storage)
            {
                apply(param, shared_storage);
            }

        private:
            /// Compute Softmax
            CUTLASS_DEVICE void apply(const Params& params, SharedStorage& shared_storage)
            {
                    ...
                    idx += ApplyShape::kColumn * kAlignment)
                {
                    if (idx_n < params.args.extent.column())
                    {
                        ...
                        FragmentSoftmax result = mul(
                            exponential(minus(convert_soft_compute(fetch), convert_norm(norm))), convert_sum(inv_sum));
    ...
    public:
        ...
        Status run(cudaStream_t stream)
        {
            ...
            Kernel<ApplyFinalReductionKernel><<<
                final_reduction_grid, final_reduction_block, sizeof(typename ApplyFinalReductionKernel::SharedStorage),
                stream
                >>>(params_.reduction);
