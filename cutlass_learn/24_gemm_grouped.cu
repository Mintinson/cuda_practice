...
struct Options
{
    ...
    // problem size bins
    std::unordered_map<cutlass::gemm::GemmCoord, std::vector<int32_t>, HashGemmCoord>
    problem_bins;
    ...
    std::vector<GroupScheduleMode> all_scheduler_modes = {
        GroupScheduleMode::kDeviceOnly, GroupScheduleMode::kHostPrecompute
    };
        ...
        if (!scheduler_mode_strs.empty())
        {
            ...
            else
            {
                for (std::string precomp_str : scheduler_mode_strs)
                {
                    ...
                    {
                        std::cerr << "Flag --scheduler-modes=all must not contain other scheduler modes in list." <<
                            std::endl;
                        error = true;
                        return;
                    }
...
public:
    ...
    cutlass::DeviceAllocation<ElementA> block_D;
    ...
    BaseTestbed(Options& options,
                cutlass::Distribution::Kind init_A = cutlass::Distribution::Uniform,
                cutlass::Distribution::Kind init_B = cutlass::Distribution::Uniform,
                cutlass::Distribution::Kind init_C = cutlass::Distribution::Uniform,
                std::uint32_t seed = 3000
    ):
        options(options), init_A(init_A), init_B(init_B), init_C(init_C), seed(seed)
    {
    }

    int problem_count() const { return options.problem_count; }
