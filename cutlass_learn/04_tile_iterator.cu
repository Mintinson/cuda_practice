...
__global__ void copy(
    ...
    typename Iterator::Element *dst_ptr,
    typename Iterator::Params src_params,
    typename Iterator::Element *src_ptr,
    cutlass::Coord<2> extent)
{
    Iterator dst_iterator(dst_params, dst_ptr, extent, threadIdx.x);
    Iterator src_iterator(src_params, src_ptr, extent, threadIdx.x);
...
cudaError_t test_tile_iterator(int M, int K)
{
    ...
    Element oob_value = static_cast<Element>(-1);

    // initialize destination tensor with all -1
    ...
    Iterator::Params dst_params(dst_tensor.layout());
    Iterator::Params src_params(src_tensor.layout());
...
int main()
{
    cudaError_t result = test_tile_iterator(57, 35);
